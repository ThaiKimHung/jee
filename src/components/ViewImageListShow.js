...
import { ActivityIndicator, FlatList, Image, Text, TouchableOpacity, View, BackHandler, StyleSheet } from 'react-native';
...
export default class ViewImageListShow extends Component {
    ...
    _renderItemImage = ({ item, index }) => {
        ...
        if (this.state.index == index)
            cusType = { padding: 2, borderRadius: 3, backgroundColor: colors.colorBGHome }
        return (
            <TouchableOpacity style={{
                ...
                <Image
                    ...
                    source={{ uri: item.url }}
                    style={{ height: sizes.reSize(80), width: sizes.reSize(80) }}
                    resizeMode="cover"
                />
            </TouchableOpacity>
        );
