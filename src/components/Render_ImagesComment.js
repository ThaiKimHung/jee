...
export default class Render_Images extends Component {
    ...
    render() {
        var { defaultImage, ImageNetWork, styImages, resizeModeFastImage, resizeModeIamge, oneImg = false } = this.props
        return (
                        ...
                        <View>
                            {
                                oneImg == true ? (
                                    <View style={{ alignItems: 'center' }}>
                                        {ImageNetWork && this.state.error == false ? (
                                            <FastImage
                                                style={[{ height: Height(10) }, styImages]}
                                                source={{
                                                    uri: ImageNetWork,
                                                    // priority: FastImage.priority.high,
                                                }}
                                                resizeMode={FastImage.resizeMode.contain}
                                                onError={async () => { await this.setState({ error: !this.state.error }), Utils.nlog('state', this.state.error) }}
                                            />
                                        ) : (
                                                null
                                            )}
