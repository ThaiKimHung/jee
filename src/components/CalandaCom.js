...
import { Image, StyleSheet, Text, TouchableOpacity, View, Platform, Animated, BackHandler } from 'react-native';
...
const Calanda = (props) => {
    ...
    useEffect(() => {
        startAnimation(0.8)
        const handlerBack = BackHandler.addEventListener('hardwareBackPress', _handlerBackButton)
        return () => {
            handlerBack.remove()
        }
    }, [])
