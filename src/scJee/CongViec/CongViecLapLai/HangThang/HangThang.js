...
class HangThang extends Component {
    constructor(props) {
        super(props);
        ...
        ROOTGlobal.GetCongViecLapLaiThang.GetCVLapLaiThang = this._getListCongViecLapLai_Thang

    }

    componentDidMount = () => {
        this._getListCongViecLapLai_Thang()
    }

    _getListCongViecLapLai_Thang = async () => {
        const { dataCV } = this.state
        let num = this.props.countCVThang
        let numTong = 0
        let res = await Get_ListCVLL()
        // console.log('res _getListCongViecLapLai_Thang', res)
        if (res.status == 1) {
            res.data.map(item => {
                if (item.frequency == 2) {
                    numTong += 1
            ...
            if (num != numTong) {
                this.props.SetCounCVLapLaiThang(numTong)
            }
            await this.setState({
                dataCV: res.data,
                refreshing: false,
    ...
    _xemChiTiet_ChinhSua = async (item) => {
        Utils.goscreen(this.nthis ? this.nthis : this, "Modal_TaoCongViecMoi", {
            type: 'Hàng tháng', onlyOne: false, nvCoSan: [], reLoad: this._getListCongViecLapLai_Thang,
            chinhSua: true, rowid: item.id_row
        })
    }

    _taoCongViecMoi = () => {
        Utils.goscreen(this.nthis ? this.nthis : this, "Modal_TaoCongViecMoi", { type: 'Hàng tháng', onlyOne: false, nvCoSan: [], reLoad: this._getListCongViecLapLai_Thang, })
    }
    ...
    render() {
        ...
        return (
            <View style={styles.container}>
                ...
                <ListData type={'Hàng tháng'} nthis={this.nthis}></ListData>
            </View>
        )
    }
...
export default Utils.connectRedux(HangThang, mapStateToProps, true)
