...
import {
    Text, TouchableOpacity, View, StyleSheet, Image, TextInput, Platform, FlatList, ActivityIndicator, BackHandler
} from 'react-native';
...
const actionSheetRef_PickTuan = createRef();
const actionSheetRef_PickThang = createRef();
...
class ListData extends Component {
    constructor(props) {
        ...
        ROOTGlobal.GetCongViecLapLaiTuan.GetCVLapLaiTuan = this._getListCongViecLapLai
    }
    ...
    _getListCongViecLapLai = async () => {
        ...
        let numTUAN = this.props.countCVTuan
        let numTHANG = this.props.countCVTuan
        let numTuan = 0
        let numThang = 0
        let res = await Get_ListCVLL()
        // console.log('res _getListCongViecLapLai', res)
        if (res.status == 1) {
            res.data.map(item => {
                if (item.frequency == 1) {
                    numTuan += 1
    ...
    _DeleteCongViec = () => {
        this.type == 'Hàng tuần' ?
            actionSheetRef_PickTuan.current?.hide()
            : actionSheetRef_PickThang.current?.hide()
    ...
    _longPress = (item) => {
        this.setState({ item: item }, () => {
            this.type == 'Hàng tuần' ?
                actionSheetRef_PickTuan.current?.show()
                : actionSheetRef_PickThang.current?.show()
        })
    }
    ...
    _xemChiTiet_ChinhSua = async (item) => {
        ...
        Utils.goscreen(this.nthis ? this.nthis : this, "Modal_TaoCongViecMoi", {
            type: this.type, onlyOne: false, nvCoSan: [], reLoad: this._getListCongViecLapLai,
    ...
    _taoCongViecMoi = () => {
        Utils.goscreen(this.nthis ? this.nthis : this, "Modal_TaoCongViecMoi", { type: this.type, onlyOne: false, nvCoSan: [], reLoad: this._getListCongViecLapLai, })
    }
    ...
    render() {
        ...
        return (
            <View style={styles.container}>
                ...
                <ActionSheet ref={this.type == 'Hàng tuần' ? actionSheetRef_PickTuan : actionSheetRef_PickThang}>
...
export default Utils.connectRedux(ListData, mapStateToProps, true)
