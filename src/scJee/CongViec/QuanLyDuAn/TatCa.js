...
import { GetActivityPhongBan, getDuAnPhongBan, getDuAnPhongBanDuocTaoBoiToi } from '../../../apis/JeePlatform/API_JeeWork/apiDuAn';
...
class TatCa extends Component {
    ...
    _DuAnPhongBan = async (page = 1) => {
        ...
        // const res = await getDuAnPhongBan(page, searchString, type, hienthi)
        const res = await getDuAnPhongBanDuocTaoBoiToi(page, searchString, type, hienthi)
        ...
        if (res.status == 1) {
            ...
            let arr = []
            res.data.map(val => {
                val.islate == 1 && val.is_manager == 1 ? arr.push(val) : null //lấy ds dự án quá hạn
            })
    ...
    _renderItem = ({ item, index }) => {
        ...
        return (
            <TouchableOpacity
                key={index}
                onPress={() => {
                    Utils.goscreen(this.nthis ? this.nthis : this, "sc_ChiTietDuAn", {
                        item: item,
                        id_row: item.id_row,
                        ...
                        <View style={{ flexDirection: 'row', justifyContent: 'space-between', flex: 1 }}>
                            <View style={{ justifyContent: 'center', paddingVertical: 5, paddingLeft: 1, width: '80%' }}>
                                ...
                                <View style={{ flexDirection: 'row', }}>
                                    ...
                                    <Text numberOfLines={1} style={{ maxWidth: "70%", fontSize: sizes.sText12, color: colors.colorGrayText, marginRight: 5 }}>{"- " + RootLang.lang.JeeWork.capnhat + " " + item.UpdatedDate}</Text>
                ...
                {item.dropDown == true ?
                    <View style={{ marginHorizontal: 20, backgroundColor: "#F2F3F5" }}>
                        ...
                        <FlatList
                            ...
                            ListEmptyComponent={_.size(item.data) == 0 && dataLoad == false ? <ListEmptyLottie style={{ justifyContent: 'center', alignItems: 'center', }} textempty={'Không có dữ liệu'} /> : null}
                        />
                    </View>
    ...
    renderAction = (action, item) => {
        switch (action) {
            case "chuyển tình trạng":
                return (
                    <>
                        <Text style={{ fontSize: 12, color: "#B3B3B3", fontWeight: "700" }}>
                            {" " + item?.action}{" "}{RootLang.lang.JeeWork.tu}{" "}
                        </Text>
            ...
            case 'tạo công việc mới':
                return (
                    <>
                        <Text style={{ fontSize: 12, color: "#B3B3B3", fontWeight: "700" }}>
                            {" " + item?.action} {RootLang.lang.JeeWork.voitencongviecla}{" "}
                        </Text>
                        <Text style={{ fontSize: 12, fontWeight: "bold" }}>
                            {item?.TenCongViec ? item.TenCongViec : ''}
                        </Text>
                    </>
                )
            default:
            ...
            <View style={{ flex: 1 }}>
                <View style={{ flex: 1, backgroundColor: "#F2F3F5" }}>
                    <View style={{
                        ...
                        {this.nthis ? null :
                            <HeaderComStack onPressLeft={() => {
                                Utils.goback(this, null)
                                ROOTGlobal.GetSoLuongNhacNho.GetSLNhacNho()
                            }}
                                nthis={this} title={RootLang.lang.JeeWork.duanquahan}
                            />
                        }
                        ...
                        <View style={{ marginVertical: 10, flexDirection: "row", justifyContent: "space-between", paddingHorizontal: 20 }}>
                            ...
                            <TouchableOpacity style={{
                                ...
                                onPress={() => {
                                    Utils.goscreen(this.nthis ? this.nthis : this, "BoLocDuAn", {
...
export default Utils.connectRedux(TatCa, mapStateToProps, true)
