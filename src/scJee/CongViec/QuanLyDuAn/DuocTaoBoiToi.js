...
import { GetActivityPhongBan, getDuAnPhongBanDuocTaoBoiToi } from '../../../apis/JeePlatform/API_JeeWork/apiDuAn';
...
class DuocTaoBoiToi extends Component {
    constructor(props) {
        super(props);
    ...
    _DuAnPhongBan = async (page = 1) => {
        ...
        if (res.status == 1) {
            ...
            let arr = []
            res.data.map(val => {
                val.is_manager == 1 ? arr.push(val) : null //lấy ds dự án quá hạn
            })
...
export default Utils.connectRedux(DuocTaoBoiToi, mapStateToProps, true)
