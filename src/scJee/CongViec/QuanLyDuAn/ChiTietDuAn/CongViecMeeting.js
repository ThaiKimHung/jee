...
class CongViecMeeting extends Component {
    constructor(props) {
        super(props);
        this.jeemeting = Utils.ngetParam(this, "jeemeting", false)
        this.reload = Utils.ngetParam(this, 'reload')
        this.state = {
            item: Utils.ngetParam(this, "item", ''),
            id_row: Utils.ngetParam(this, "id_row", ''),
            itemprop: Utils.ngetParam(this, "item", ''),
            listStatus: [],
            refreshing: false,
            empty: true,
            showload: false,
    ...
    async componentDidMount() {
        if (!this.state.item) {
            this.setState({ item: this.item }, async () => {
                nthisLoading.show()
    ...
    _renderItemChild({ item, index }) {
        ...
        return (
            <TouchableOpacity
                key={index}
                onLongPress={() => {
                    Utils.showMsgBoxYesNo(this ? this : this, RootLang.lang.JeeWork.thongbao, RootLang.lang.JeeWork.bancomuonxoacongviec, RootLang.lang.JeeWork.xoacongviec, RootLang.lang.JeeWork.dong, () => { this._DeleteCongViec(item.id_row, index) })
                }}
                onPress={() => {
                    Utils.goscreen(this ? this : this, "sc_ChiTietCongViecCaNhan", {
        ...
        return (
            <View key={index} style={{ marginHorizontal: 3, marginVertical: 1 }}>
                <TouchableOpacity
                    ...
                    style={{ flexDirection: 'row', justifyContent: 'space-between', padding: 10, backgroundColor: this.Opaciti_color(item.color), height: Height(5), borderRadius: 5 }}>
                ...
                {!item.dropdown == true ?
                    <View style={{ flex: 1, paddingHorizontal: 10 }}>
                        <FlatList
                            ...
                            ListEmptyComponent={item.datawork.length == 0 ? <View style={{ width: '100%', height: 50, justifyContent: 'center', alignItems: 'center' }}><Text style={{ color: colors.black_50, fontSize: reText(15) }}>{'Không có dữ liệu'}</Text></View> : null}
    ...
    render() {
        ...
        return (
            ...
            <TouchableWithoutFeedback
                ...
                <View style={{ flex: 1 }}>
                    <HeaderAnimationJee isGoBack={true} OnGoBack={this._goback} nthis={this} Screen={true} title={itemprop.title} />
                    <ScrollView nestedScrollEnabled={true} keyboardShouldPersistTaps={"handled"} style={{ backgroundColor: colors.backgroudJeeHR, marginTop: 3, flex: 1 }}>
                        <View style={{ backgroundColor: colors.backgroudJeeHR, paddingLeft: 10, flexDirection: 'row', width: Width(100), paddingVertical: 5 }}>
                            <View style={{ justifyContent: 'center' }}>
                                <TouchableOpacity
                                    onPress={() => {
                                        Utils.goscreen(this, "TaoGiaoViec", {
                                            callback: this.CallBack,
                                            project: {
                            ...
                            <View style={{ flexDirection: 'row', justifyContent: 'center' }}>
                                <TouchableOpacity
                                    onPress={() => { Utils.goscreen(this, "sc_BoLocChiTietDuAn", { callback: this.CallBackBoLoc }) }}
                        ...
                        <View style={{ flex: 1, marginBottom: Platform.OS == 'ios' ? 20 : 5 }}>
                            <FlatList
                                ...
                                style={{ marginTop: 3, backgroundColor: colors.backgroudJeeHR }}
                                data={listCongViec}
...
export default Utils.connectRedux(CongViecMeeting, mapStateToProps, true)
