...
class DenHan_HetHan extends Component {
    constructor(props) {
        super(props);
    ...
    async componentDidMount() {
        ...
        await this._GetDSCongViec().then(res => {
            if (res == true) {
    ...
    _GetDSCongViec = async () => {
        ...
        const res = await getDSCongViecCaNhan(searchString, _dateDk, _dateKt, typeChoice)
        if (res.status == 1) {
            this.refLoading.current.hide()
            ...
            // await this.setState({ dataCongViec: _.flattenDeep(this.state.dataCongViecTemp) })
            // for (let y = 0; y <= _.size(this.state.dataCongViec); y++) {
            //     for (let z = 0; z < _.size(this.state.dataCongViec[y]?.Status); z++) {
            //         if (this.state.dataCongViec[y]?.status == this.state.dataCongViec[y]?.Status[z]?.id_row) {
            //             const newItem = Object.assign({}, this.state.dataCongViec[y], { StatusCongViec: this.state.dataCongViec[y]?.Status[z] });
            //             this.setState({
            //                 dataCV: _.concat(this.state.dataCV, newItem),
            //                 refreshing: false
    ...
    _renderItem = ({ item, index }) => {
        return (
            <>
                {
                    item.trehan == 1 && item.closed != true ?
                        <TouchableOpacity
    ...
    render() {
        var { dataCV, refreshing, dataLoad, searchString } = this.state
        return (
            <View style={{ flex: 1 }}>
                <View style={{ flex: 1, backgroundColor: "#F2F3F5" }}>
                    <View style={{
                        ...
                        {this.nthis ? null :
                            <HeaderComStack onPressLeft={() => { Utils.goback(this, null) }}
                                nthis={this} title={RootLang.lang.JeeWork.congviecquahan}
                            />
                        }
                        <View style={{ marginVertical: 10, flexDirection: "row", justifyContent: "space-between", paddingHorizontal: 20 }}>
                            ...
                            <TouchableOpacity style={{
                                ...
                                onPress={() => {
                                    Utils.goscreen(this.nthis ? this.nthis : this, "sc_BoLoc", {
                    ...
                    <ButtonCom
                        text={RootLang.lang.JeeWork.giaocongviecmoi}
                        style={{
                            backgroundColor: colors.white,
                        ...
                        styleButton={{ marginHorizontal: 10, bottom: 0, marginTop: 10, marginBottom: Platform.OS == 'ios' ? 20 : 10 }}
                        txtStyle={{ color: colors.colorJeeNew.colorBlueHome }}
                        onPress={() => {
                            Utils.goscreen(this.nthis ? this.nthis : this, "TaoGiaoViec", {
                                callback: this.CallBack
                            })
                        }}
                    />
...
export default Utils.connectRedux(DenHan_HetHan, mapStateToProps, true)