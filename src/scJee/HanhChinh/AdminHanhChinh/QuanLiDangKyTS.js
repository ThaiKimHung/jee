...
import { DS_DatPhongHop, Get_ListTaiSan } from '../../../apis/JeePlatform/API_JeeHanhChanh/apiJeeHanhChanh'
...
export class QuanLiDangKyTS extends Component {
    constructor(props) {
        super(props)
        this.idUser = ''
        ...
        this.state = {
            ...
            dataStatus: [
                { id: -1, title: RootLang.lang.thongbaochung.tatca, color: colors.orange },
                { id: 0, title: RootLang.lang.thongbaochung.choxacnhan, color: '#C0C0C0' },
                { id: 1, title: RootLang.lang.thongbaochung.dadat, color: '#77BBEE' },
                { id: 2, title: RootLang.lang.thongbaochung.dahuy, color: '#FF7676' }
            ],
            selectStatus: { id: -1, title: RootLang.lang.thongbaochung.tatca, color: colors.orange },
    ...
    Get_ListTaiSan = async () => {
        ...
        if (res.status == 1 && res.data) {
            this.setState({
                dataTaiSan: res.data, selectTaiSan: res.data[0]
            }, () => {
    ...
    GetCalendarTaiSan = async (month = true, status = this.state.selectStatus.id) => {
        this.refLoading.current.show()
        this.setState({ events: [] })
        let res = await DS_DatPhongHop(this.state.selectTaiSan.RowID, status, this.state.dateSelect, month)
        if (res.status == 1) {
    ...
    renderEvent = (event, touchableOpacityProps) => {
        return (
            <TouchableOpacity key={event.start + event.title} {...touchableOpacityProps}
                onPress={() => Utils.goscreen(this, 'FormChiTiet_Admin', { event: event, callback: () => this.state.modeCalendar == 'keylichbieu' ? this.GetCalendarTaiSan(false) : this.GetCalendarTaiSan() })}>
    ...
    renderLichBieu = ({ item, index }) => {
        ...
        return (
            <View key={index} style={{ marginBottom: index == (this.state.events.length - 1) ? 30 : 0 }}>
                ...
                <TouchableOpacity onPress={() => Utils.goscreen(this, 'FormChiTiet_Admin', {
                    event: item,
    ...
    goFormDK = (e) => {
        Utils.goscreen(this, 'FormDangKyTS', { selectTaiSan: this.state.selectTaiSan, valDate: e, callback: () => this.state.modeCalendar == 'keylichbieu' ? this.GetCalendarTaiSan(false) : this.GetCalendarTaiSan() })
    }
    ...
    changeStatus = (val) => {
        this.setState({ selectStatus: val }, () => this.GetCalendarTaiSan(true, val.id))
    }

    render() {
        const { events, modeCalendar, dataModeCalendar, dateSelect, selectTaiSan, dataStatus, selectStatus, checkSelectTime } = this.state
        ...
        return (
            <View style={{ flex: 1 }}>
                <View style={{ flex: 1, backgroundColor: colors.white }}>
                    <HeaderComStackV2
                        nthis={this}
                        title={RootLang.lang.thongbaochung.quanlytaisandungdung}
                        iconLeft={Images.ImageJee.icBack}
                        onPressLeft={this._goback}
                    />
                    <View style={{ width: Width(100), height: 0.5, backgroundColor: colors.black_20_2, marginBottom: 5 }} />
                    <View style={{ flexDirection: 'row', justifyContent: 'space-around' }}>
                        {dataStatus.map((item, index) => {
                            return (
                                <TouchableOpacity onPress={() => this.changeStatus(item)} key={index} style={{ backgroundColor: selectStatus.id == item.id ? item.color : null, width: index == 1 ? Width(30) : Width(20), justifyContent: 'center', alignItems: 'center', borderWidth: 1, borderRadius: 20, paddingVertical: 8, paddingHorizontal: 5, borderColor: selectStatus.id == item.id ? item.color : colors.black_20_2 }}>
                                    <Text style={{ fontSize: reText(13), color: selectStatus.id == item.id ? colors.white : colors.black_50 }}>{item.title}</Text>
                                </TouchableOpacity>
                            )
                        })}
                    </View>
                    <View style={{ width: Width(100), height: 0.5, backgroundColor: colors.black_20_2, marginVertical: 5 }} />
...
export default Utils.connectRedux(QuanLiDangKyTS, mapStateToProps, true)
