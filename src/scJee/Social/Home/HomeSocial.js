...
import {
    addGhimBaiDang, BaiDangLike, BaiDang_LuuTru, deleteBaiDang, LoadDanhSachBaiDang, AddTinNoiBat, On_OffThongBao,
    HuyTinNoiBat
...
class HomeSocial extends Component {
    constructor(props) {
        ...
        this.state = {
            ...
            empty: true,
            role: { "roles": "1,3" },
            loading: true // false : mở, true: tắt
        };
        this.allPage = 1;
        this.types = [
            { key: '1', type: 'Like' },
        ...
        ROOTGlobal.LoadDanhSachBaiDang.LoadDSBaiDang = this._loadDanhSachBaiDang_lai

        this.scrollValue = 0
        this.animatedButton = new Animated.Value(0)
        this.translateButton = this.animatedButton.interpolate({
            inputRange: [0, 1],
            outputRange: [0, -HEIGHT_BUTTON],
        });
    }
    componentDidMount = async () => {
        ...
        this._loadDanhSachBaiDang().then(res => {
            if (res == true) {
    ...
    _goModalBinhLuan = (topicid, idBaiDang) => {
        Utils.goscreen(this.nthis, 'sc_BinhLuan', { nthis: this.nthis, idbaidang: topicid, idnoti: idBaiDang })
    }

    _taoBaiDang = () => {
        Utils.goscreen(this.nthis, 'sc_TaoBaiDangTheoLoai', { nthis: this.nthis })
    }

    _loadDanhSachBaiDang = async (page = 1) => {
        const { listBaiDang } = this.state
        let dataBaiDang = await Utils.ngetStorage(nkey.listBaiDang, [])
        if (dataBaiDang.length == 0) {
            this.setState({ loading: false, })
            // this.refLoading.current.show()
        }
        else {
            this.setState({ loading: true, empty: true, listBaiDang: dataBaiDang, })
        }
        let res = await LoadDanhSachBaiDang('home', '', '', page);
        ...
        if (res.status == 1) {
            ...
            await this.setState({ listBaiDang: tempData.map(obj => ({ ...obj, check: false })), _page: page, refreshing: false, showload: false, empty: true, loading: true })
            await Utils.nsetStorage(nkey.listBaiDang, this.state.listBaiDang)
        } else {
            this.setState({ refreshing: false, showload: false, empty: false, listBaiDang: [] })
    ...
    _loadDanhSachBaiDang_lai = async (page = 1) => {
        const { listBaiDang, listComment } = this.state
        this.refLoading.current.show()
        this.setState({ empty: false })
        let res = await LoadDanhSachBaiDang('home', '', '', page);
        // Utils.nlog("list bài đăng lại-------------------------", res)
        if (res.status == 1) {
            let tempData = listBaiDang;
            this.allPage = res.page.AllPage;
            tempData = res.data;
            this.refLoading.current.hide()
            await this.setState({ listBaiDang: tempData.map(obj => ({ ...obj, check: false })), _page: 1, refreshing: false, showload: false, empty: false })
            await Utils.nsetStorage(nkey.listBaiDang, this.state.listBaiDang)
        } else {
            this.refLoading.current.hide()
            this.setState({ refreshing: false, showload: false, empty: true, listBaiDang: [] })
            // if (res.error.message != "Không có dữ liệu!") {
            //     UtilsApp.MessageShow(RootLang.lang.thongbaochung.thongbao, res.error ? res.error.message : RootLang.lang.thongbaochung.loilaydulieu, 2)
            // }
        }
    ...
    _editBaiDang = async () => {
        actionSheetRef.current?.hide();
        Utils.goscreen(this.nthis, 'Modal_EditBaiDang', { nthis: this.nthis, data: this.state.item })
    }

    _ghimBaiDAng = async (type = false) => {
        this.refLoading.current.show()
        let res = await addGhimBaiDang(this.state.item.Id_BaiDang);
        // Utils.nlog('ress _ghimBaiDAng', res)
        if (res.status == 1) {
            actionSheetRef.current?.hide()
    ...
    getReactionJson = type => {
        switch (type) {
            case 'Like':
                return require('./LottieEmotion/like.json');
            case 'Love':
                return require('./LottieEmotion/love.json');
            case 'Haha':
                return require('./LottieEmotion/haha.json');
            case 'Wow':
                return require('./LottieEmotion/wow.json');
            case 'Sad':
                return require('./LottieEmotion/sad.json');
            case 'Care':
                return require('./LottieEmotion/care.json');
            case 'Angry':
                return require('./LottieEmotion/angry.json');
        }
    };
            ...
            <TouchableWithoutFeedback onPress={() => { this.setState({ showReact: null }) }}>
                <View style={styles.container}>
                            ...
                            <View style={styles.con_khungnhom}>
                                <View style={{ flex: 1, }}>
                                    <View style={[styles.con_khungten, {}]}>
                                        <Text style={styles.text_name}>{item?.User_DangBai[0]?.Fullname ? item?.User_DangBai[0]?.Fullname : '...'}</Text>
                                        {
                                            item.Group.length > 0 ? (
                                                <View style={{ flex: 1, }}>
                                ...
                                </View>
                                {
                                    item.CreatedBy == this.state.id_User ? (
                                        <TouchableOpacity style={[{ alignItems: 'center', justifyContent: 'center', height: Height(3), width: Width(6), paddingBottom: 10, }]}
                                            onPress={() => { actionSheetRef.current?.show(), this.setState({ item: item }) }} >
                    ...
                    <View style={{ flexDirection: 'row', flex: 1, justifyContent: 'space-between', height: Height(4) }}>
                        ...
                        <TouchableOpacity onPress={() => this._goModalBinhLuan(item.objectid, item.Id_BaiDang)} style={{ width: Width(32), justifyContent: 'center', alignItems: 'center' }}>
                            <View style={{ flexDirection: 'row', justifyContent: 'center', alignItems: 'center' }}>
    ...
    render() {
        const { listBaiDang, showload, refreshing, item, id_User, empty, loading } = this.state;
        return (
            <View style={{ flex: 1, backgroundColor: '#E4E6EB' }} >
                {/* <ButtonTaoBaiDang onPress={this._taoBaiDang}></ButtonTaoBaiDang> */}
                {/* <Animated.View style={{ transform: [{ translateY: this.translateButton }], position: 'absolute', zIndex: 1, elevation: 1 }}>
                    <TouchableOpacity style={{ justifyContent: 'center', alignItems: 'center' }} onPress={this._taoBaiDang}>
                        <Image source={Images.ImageJee.icTouchSocail}
                            resizeMode='cover'
                            style={{ width: Width(100), height: HEIGHT_BUTTON }}>
                        </Image>
                        <Text style={{ color: colors.white, fontSize: reText(15), position: 'absolute' }}>{RootLang.lang.JeeSocial.taobaidangmoi}</Text>
                    </TouchableOpacity>
                </Animated.View> */}
                <View style={{ flex: 1, paddingBottom: paddingBotX, }}>
                    {
                        loading == false ? (
                            <Loading soLuongRender={[0, 1, 2, 3]} stylesLoading={{}}  ></Loading>
                        ) : (
                                <FlatList
                                    // keyboardShouldPersistTaps='always'  khi thao tác keybroad sẽ k bị tắt bàn phím 
                                    ...
                                    ListEmptyComponent={this.state.listBaiDang.length == 0 ? <ListEmptyLottie_Social style={{ justifyContent: 'center', alignItems: 'center', }} textempty={empty == false ? RootLang.lang.JeeSocial.khongcobaidang : null} styleText={{ marginTop: -50 }} /> : null}
                                    ListFooterComponent={showload ? <ActivityIndicator size='small' /> : null}
                                    ListHeaderComponent={
                                        <ButtonTaoBaiDang onPress={this._taoBaiDang}></ButtonTaoBaiDang>
                                    }
                                // stickyHeaderIndices={[0]} // không cho header di chuyển
                                />
                            )
                    }
                </View >
...
export default Utils.connectRedux(HomeSocial, mapStateToProps, true)
